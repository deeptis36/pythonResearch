...
X_train, X_test, y_train, y_test = train_test_split(df['job_description'], df['skills'], test_size=0.3, random_state=42)

# Create a pipeline for the model
pipeline = Pipeline([
    ('tfidf', TfidfVectorizer()),  # Convert text to TF-IDF features
    ('classifier', LogisticRegression(max_iter=1000))  # Logistic Regression classifier with more iterations
])

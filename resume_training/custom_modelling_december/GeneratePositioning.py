...
from resumeText import extract_words_between
...
def get_employer_position_from_text(key, text):
    ...
    match = re.search(pattern, text, re.IGNORECASE)
    # print(f"{key}, {match.start()},{match.end}")
    if match:
        return [match.start(), match.end()]
...
def find_keyword_location(key_word, text):
    """
    Finds the start and end index of a keyword in a given text.

    Args:
        key_word (str): The word to search for.
        text (str): The text to search within.

    Returns:
        list: A list of tuples, where each tuple contains the start and end index of the keyword in the text.
              Returns an empty list if the keyword is not found.
    """
    if not key_word or not text:
        return []

    import re

    # Escape special characters in the key_word for a safe regex search
    escaped_keyword = re.escape(key_word)

    # Use re.finditer to find whole word occurrences of the keyword in the text
...
def get_employer_position(key, text):
    ...
    if employer_position is not None:
        ...
        proces_key = extract_words_between(text, key_words[0], key_words[-1])
        employer_position = get_employer_position_from_text(proces_key, text)
        if employer_position is not None:
            return employer_position
    
    # exit("get employer position terminated")
...
def get_position_entity(entities,text):
    ...
    for entity in entities:
        ...
        for key in entity:
            ...
            # print(f"location_role {location_role}")
            
            location_date = get_date_position(date,text)
            # print(date)
            # print(f"location_date {location_date}")
            

            location_employer = get_employer_position(employer,text)
            # if location_employer is None:
                # print(employer)
                # print(f"location_employer {location_employer}")
            # continue

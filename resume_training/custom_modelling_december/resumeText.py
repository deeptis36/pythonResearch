...
def extract_words_between(text, start_word, end_word):
    # print(start_word)
    # print(end_word)
    # exit("get employer position failed")
    # print(start_word, end_word)
    try:
        # Ensure case-insensitive matching if needed
        start_index = text.lower().index(start_word.lower()) + len(start_word)
       
        # Find the end index before the end_word
        end_index = text.lower().index(end_word.lower(), start_index)
     
        # Extract the substring
        substring = text[start_index:end_index].strip()
        processed_string = start_word+" "+substring+" "+end_word
     
        return processed_string
    except ValueError:
        print("Start or end word not found in the text")
        return None  # Return "Start or end word not found in the text"